...
bgRemoval.post('/api/update-mask/:sku', async (c) => {
    try {
        const sku = c.req.param('sku');
        const companyId = getCompanyId(c);
        const body = await c.req.json();
...
bgRemoval.post('/api/save-mask/:sku', async (c) => {
    try {
        const sku = c.req.param('sku');
        const companyId = getCompanyId(c);
...
bgRemoval.post('/api/regenerate-with-mask/:sku', async (c) => {
    try {
        const sku = c.req.param('sku');
        const companyId = getCompanyId(c);
        ...
        // Note: Client-side regeneration is recommended for better performance
        // This endpoint is for future server-side implementation if needed
        
        // For now, return URLs for client-side processing
        return c.json({
            success: true,
            sku,
            originalUrl,
