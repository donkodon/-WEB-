...
export class DashboardRepository implements IDashboardRepository {
  async fetchDashboardProducts(
    ...
    for (const item of itemsResult.results) {
      ...
      for (let i = 0; i < imageUrls.length; i++) {
        ...
        productData.images.push({
          ...
          created_at: new Date().toISOString(), filename, sku, sequence: i + 1, is_main: i === 0, updated_at: updatedAt,
        })
      }

      const annotatedImageUrl = pi.annotated_image_url as string | undefined
      const maskImageUrl = pi.mask_image_url as string | null
      if (annotatedImageUrl && productData.has_measurement) {
        ...
        productData.images.push({
          ...
          sku, sequence: imageUrls.length + 1, is_main: false, is_measurement: true, updated_at: updatedAt,
        })
      }
    }
    ...
    return { products, pagination: { page, perPage, total, totalPages, hasNext: page < totalPages, hasPrev: page > 1 } }
  }
